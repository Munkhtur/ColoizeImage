...
protoFile = "./app/models/colorization_deploy_v2.prototxt"
weightsFile = "./app/models/colorization_release_v2.caffemodel"
...
def colorize(imagepath):
    ...
    numpy_file = np.load('./app/models/pts_in_hull.npy')
    ...
    lab_output = np.concatenate(
        (l_channel[:, :, np.newaxis], ab_channel_us), axis=2)  # concatenate with original image L
    bgr_output = np.clip(cv.cvtColor(lab_output, cv.COLOR_Lab2BGR), 0, 1)

    random_hex = secrets.token_hex(8)
    cv.imwrite("./app/static/result/" + random_hex + '.png',
